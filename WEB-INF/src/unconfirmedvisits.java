...
public class unconfirmedvisits extends HttpServlet{
    public boolean fetch(int sup_id, HttpServletRequest request, HttpServletResponse response){
      ...
      try {
        ...
        request.getRequestDispatcher("Supervisor/unconfirmedvisits.jsp").forward(request, response);
      }
    ...
    public boolean confirmvisit(int visit_id, HttpServletRequest request, HttpServletResponse response){
        // 2. Define the Connection URL
        String url = "jdbc:mysql://localhost/NAD";
    ...
    throws ServletException, IOException{
        ...
        if(confirmvisit(visit_id, request, response)){
            request.setAttribute("go", go);
            request.getRequestDispatcher("confirmedvisits").forward(request, response);
        }else{
            request.setAttribute("dont", dont);
